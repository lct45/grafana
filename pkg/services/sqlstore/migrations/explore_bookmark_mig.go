...
func addExploreBookmarkMigrations(mg *Migrator) {
	exploreBookmarkV1 := Table{
		...
		Columns: []*Column{
			...
			{Name: "time_from", Type: DB_Text, Nullable: false},
			{Name: "time_to", Type: DB_Text, Nullable: false},
	...
	mg.AddMigration("add index explore_bookmark.org_id-user_id", NewAddIndexMigration(exploreBookmarkV1, exploreBookmarkV1.Indices[0]))
	mg.AddMigration("add unique index explore_bookmark.uid", NewAddIndexMigration(exploreBookmarkV1, exploreBookmarkV1.Indices[1]))
}
