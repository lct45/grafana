...
const recentItemsPath = "/api/user/recent-items"

func (s *RecentItemsService) registerAPIEndpoints() {
	s.routeRegister.Group(recentItemsPath, func(items routing.RouteRegister) {
		items.Post("/", routing.Wrap(s.createHandler))
		items.Get("/", routing.Wrap(s.listHandler))
		items.Patch("/:uid", routing.Wrap(s.patchHandler))
		items.Delete("/:uid", routing.Wrap(s.deleteHandler))
	}, middleware.ReqSignedInNoAnonymous)
}

// swagger:route POST /user/recent-items signed_in_user createRecentItem
//
...
// swagger:route GET /user/recent-items signed_in_user listRecentItems
//
...
func (s *RecentItemsService) listHandler(c *contextmodel.ReqContext) response.Response {
	...
	items, err := s.List(c.Req.Context(), c.SignedInUser, limit, c.Query("resourceType"))
	if err != nil {
...
// swagger:route PATCH /user/recent-items/{uid} signed_in_user patchRecentItem
//
...
// swagger:route DELETE /user/recent-items/{uid} signed_in_user deleteRecentItem
//
...
func (s *RecentItemsService) deleteHandler(c *contextmodel.ReqContext) response.Response {
	if err := s.Delete(c.Req.Context(), c.SignedInUser, web.Params(c.Req)[":uid"]); err != nil {
