...
- **Node.js v24.x** (see `.nvmrc` for exact version). Use `nvm install` / `nvm use` to match. Gotcha: a system `/exec-daemon/node` (v22) sits early on `PATH` and shadows nvm's node in non-login shells, so a bare `node --version` may report v22. `~/.bashrc` is configured to prepend nvm's node 24, so **login shells** (`bash -l`, the default for interactive/tmux sessions) already resolve node 24 — prefer login shells, or run `nvm use` first. Node 22 also satisfies the repo's `engines` (`>=22 <25`), so installs still work under either, but node 24 is preferred.
