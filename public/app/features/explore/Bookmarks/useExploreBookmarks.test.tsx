...
const mockBookmark = {
  ...
  timeFrom: String(dateTime('2024-01-15T10:00:00.000Z').valueOf()),
  timeTo: String(dateTime('2024-01-15T11:00:00.000Z').valueOf()),
  createdAt: 1,
  updatedAt: 1,
};
...
describe('useExploreBookmarks', () => {
  ...
  it('saves absolute DateTime ranges via toURLRange epoch millis, not String(DateTime)', async () => {
    ...
    const call = (exploreBookmarkApi.createExploreBookmark as jest.Mock).mock.calls[0][0];
    expect(Number(call.timeFrom)).toBe(from.valueOf());
    expect(Number(call.timeTo)).toBe(to.valueOf());
  });
  ...
  it('opens a bookmark by applying parsed time then queries before a single run', async () => {
    ...
    expect(pane.range.raw.from.valueOf()).toBe(Number(mockBookmark.timeFrom));
    expect(pane.range.raw.to.valueOf()).toBe(Number(mockBookmark.timeTo));
  });
